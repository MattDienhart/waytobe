...
function getJobTitles(res, mysql, context, complete) {
  mysql.pool.query("SELECT id, jobsName, jobDescription FROM Jobs", function(error, results, fields){
    if(error){
...
function getJobIndustries(res, mysql, context, complete) {
  mysql.pool.query("SELECT id, industryName FROM Industries", function(error, results, fields){
    if(error){
      res.write(JSON.stringify(error));
      res.end();
    }
...
app.get("/", function (req, res) {
  ...
  getJobTitles(res, mysql, context, complete);
  getJobIndustries(res, mysql, context, complete);
  
  function complete(){
    callbackCount++;
    if(callbackCount >= 2){
      res.render('home_matt', context);
    }
  }
});
  ...
  sql = mysql.pool.query(sql, inserts, function(error, results, fields){
    if(error){
      ...
      function complete(){
        callbackCount++;
        if(callbackCount >= 2){
          res.render('skills_needed', context);
        }
      }
    }
  });
